...
# Define paths
raw_clean_file = "data/raw/cleanQs_cleaned.csv"
raw_typo_file = "data/raw/typoQs_cleaned.csv"
output_clean_issues_file = "data/preprocessed/cleanQs_issues.csv"
output_typo_issues_file = "data/preprocessed/typoQs_issues.csv"
...
def detect_issues(sentence):
    ...
    if sentence.isupper():  # Entirely uppercase (shouting)
...
#    regex_pattern = r'[^\w\s.,?!\'"()\[\]{}<>:;\-\—/_%&*=+@#$]'
    ...
    #if re.search(r',[^ ]', sentence) or re.search(r',\s{2,}', sentence):  # Misplaced commas
    if re.search(regex_pattern, sentence):
        issues.append("Misplaced commas")

    if re.search(r'""', sentence):  # Double double quotes (""), likely formatting issues
